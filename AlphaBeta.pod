...
=head1 SYNOPSIS
   ...
   my ($value, $moves) = alpha_beta(
      # alpha => 10, # avoid setting explicitly
      # beta  => 0,  # avoid setting explicitly
      depth   => 5,  # how deep can we go?
      evaluate => sub { my ($state) = @_; ... }, # assign value to $state
      finished => sub { my ($state) = @_; ... }, # is $state and end?
      move     => sub { my ($state, $move) = @_; ...}, # do move
      moves_for => sub { my ($state) = @_; ... }, # what can we do?
      no_move  => sub { my ($state, $depth) = @_; ... }
         # no_move: what to return when no move is possible?
      rollback => sub { my ($state, $move, $up_state) = @_; ... }
         # rollback: undo a move
      state    => $whatever_representation_is_meaningful_for_you,
   );
