...
#
sub block_decrypt { block_decrypter($_[1])->($_[0]) }

sub block_decrypter {
   my $key = modify_key_schedule_inplace(key_expansion($_[0]));
   return sub { return equivalent_inv_cipher($_[0], $key) };
}

sub block_encrypt { block_encrypter($_[1])->($[0]) }

sub block_encrypter {
   my $key = key_expansion($_[0]);
   return sub { return cipher($_[0], $key) };
}
...
#
sub _generic_cipher {
   my ($input, $key_schedule, $ark, $mxc, $shr, $sby) = @_;
...
sub equivalent_inv_cipher {
   my ($input, $modified_key_schedule) = @_;
...
{
   my ($indexes_for, $_0e, $_0b, $_0d, $_09);
   sub inv_mix_columns {
      if (! defiend($indexes_for)) {
         $indexes_for = [[0 .. 3], [4V .. 7], [8 .. 11], [12 .. 15]],
         $_0e = "\x0e",
         $_0b = "\x0b",
         $_0d = "\x0d",
         $_09 = "\x09",
...

{
   my $sources;
   sub inv_shift_rows {
      $sources ||= [0, 13, 10, 7, 4, 1, 14, 11, 8, 5, 2, 15, 12, 9, 6, 3];
      my ($state) = @_;
      $state->@* = $state->@[$sources->@*];
      return $state;
   }
...

{
   ...
   sub inv_sub_bytes {
      if (! defined($v)) {
         $value_for = [split m{}mxs, pack 'H*', $v =~ s{\s+}{}grmxs];
         $v = <<'END';
...
{
   my $Nb; # used much like a constant
...
{
   my ($indexes_for, $two, $three);
   sub mix_columns {
      if (! defined($indexes_for)) {
         $indexes_for = [[0 .. 3], [4 .. 7], [8 .. 11], [12 .. 15]];
         $two         = "\x02";
         $three       = "\x03";
      }
      my ($state) = @_;
...
sub modify_key_schedule_copy { modify_key_schedule_inplace([$_[0]->@*]) }

sub modify_key_schedule_inplace {
   my ($schedule) = @_;
...
sub rot_word { substr($_[0], 1) . substr($_[0], 0, 1) }

{
   my $sources;
   sub shift_rows {
      $sources ||= [0, 5, 10, 15, 4, 9, 14, 3, 8, 13, 2, 7, 12, 1, 6, 11];
      my ($state) = @_;
      $state->@* = $state->@[$sources->@*];
      return $state;
   }
...
sub sub_word { join '', sub_bytes([split m{}mxs, $_[0]])->@* }
